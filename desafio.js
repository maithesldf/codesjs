...
console.log(paisAfricano.country); 
...
const medalhasPorPais = function (paises, medalhas){
    ...
    medalhas = olympicsMedalTable.customMap(i => (i.gold + i.silver + i.bronze));
    const x = medalhasPorPais(paises, medalhas);
    for(i = 0; i < x.length; i++){
        if (i % 2 == 0){
            console.log(`O país ${x[i]} possui ${x[i+1]} medalhas.`);
        }
    }
...
console.log(`Os países com pelo menos 10 medalhas de ouro são: ${paisesCom10MedalhasOuroNoMinimo.customMap(i => i.country)}`);

// 4 - Crie um algoritmo para encontrar os países que conquistaram no minímo 30 medalhas (Ouro, Prata e Bronze)
const paisesCom30Medas7lhasNoMinimo =  olympicsMedalTable.customFilter(i => (i.gold + i.silver + i.bronze) >= 30);
console.log(`Os países com pelo menos 30 medalhas são: ${paisesCom30Medas7lhasNoMinimo.customMap(i => i.country)}`);
...
console.log(`A América do Sul ganhou ${paisesComPeloMenos20MedalhasDeOUro} medalhas de ouro`);
if (paisesComPeloMenos20MedalhasDeOUro < 20){
    console.log('A América do Sul ganhou menos de 20 medalhas de ouro.')
}